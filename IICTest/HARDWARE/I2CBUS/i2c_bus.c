...
void i2c_bus_init(void){
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//GPIO_Mode_IN;
...
int IIC_SendString(uint8_t sensor_addr,uint8_t* buffer,uint8_t lenght)
{
	uint8_t count=0;
	i2c_bus_probe();
	delay_us(timeSlot);
	...
	for(count=0;count<lenght;count++)
	{
		if(i2c_bus_write_byte(buffer[count])==0)                    
		{
			i2c_bus_stop(); 
			i2c_bus_release();		
			return 3;
